  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-6 px-6 py-12">
      <header className="flex flex-col gap-1">
        ...
        <span className="font-mono text-xs uppercase tracking-widest text-ink-soft">Fase 6 · Dinero</span>
        <h1 className="font-display text-3xl font-semibold text-ink">{modulo?.nombre ?? "Ahorro con metas"}</h1>
      ...
      <div className="flex flex-col gap-3">
        {(retos ?? []).map((reto, i) => {
          ...
          return (
            <Link key={reto.id} href={`/dashboard/modulos/ahorro-metas/${reto.slug}`}>
              <Card className="transition-colors hover:border-ink/30">
                <CardContent className="flex items-center justify-between gap-3 p-4">
                  ...
                  {completado ? (
                    <Badge tone="growth">Meta activa · {progreso?.puntaje}/100</Badge>
                  ) : (
